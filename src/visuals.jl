#=
Borrowed from Dojo.jl (https://github.com/dojo-sim/Dojo.jl/blob/main/src/visuals/set.jl)
=#
...
function set_floor!(vis::Visualizer;
	    ...
	    axis::Bool=false,
	    grid::Bool=false)
    image = PngImage(joinpath(@__DIR__, "assets", imagename))
...
function set_background!(vis::Visualizer; top_color=RGBA(0,0,0.0), bottom_color=RGBA(1,1,1.0))
    setprop!(vis["/Background"], "top_color", top_color)
    setprop!(vis["/Background"], "bottom_color", bottom_color)
end

function initialize_arena!(vis::Visualizer)
    set_background!(vis)
    set_light!(vis)
    set_floor!(vis)
